...
{
    /**
     * Array representation on template hierarchy.
     * Format is:
     *
     * branch name => [
     *      array of possible templates,
     *      boolean indicating templates has variable names
     * ]
     *
     * @var array
     */
    private static $branches = [
        ...
        'tax'               => [
            ['taxonomy-{{tax}}-{{slug}}', 'taxonomy-{{tax}}', 'taxonomy'],
            true,
        ],
        'attachment'        => [
            ['attachment'],
        ...
        'single'            => [
            ['single-{{slug}}', 'single', 'singular'],
            true,
        ],
        'page'              => [
            ['page-{{slug}}', 'page-{{id}}', 'page', 'singular'],
            true,
        ],
        'category'          => [
     ...
     * If template is found (callback return something not empty) method return whatever is
     ...
     * @param  string $branch
     * @return array
     * @see getPostTypeArchiveBranches()
     * @see getArchiveBranches()
     * @see getPageBranches()
     * @see getAttachmentBranches()
     */
    private function typeBranches($branch)
    {
        $branches = $this->getBranches();
        $templates = $branches[$branch][0];
        $cb = 'get'.str_replace(' ', '', ucwords(str_replace('_', ' ', $branch))).'Branches';

        return method_exists($this, $cb) ? $this->$cb($templates) : $templates;
    }
     ...
     * @return array
     */
    private function getPostTypeArchiveBranches(array $templates)
    {
     ...
     * @return array
     */
    private function getArchiveBranches(array $templates)
    {
     ...
     * @return array
     */
    private function getPageBranches(array $templates)
    {
        ...
        if (! empty($file) && validate_file($file) === 0) {
            ...
            array_unshift($templates, substr($file, 0, $cut < 0 ? $cut - 1 : strlen($file)));
        }
    ...
    private function getAttachmentBranches()
    {
        ...
        $templates = [$type[0]];
        if (! empty($type[1])) {
            $templates = array_merge($templates, [$type[1], "{$type[0]}_{$type[1]}"]);
        }
